...
type HuPaiSeat struct {
	...
	PiaoScore     string                          `json:"piaoScore"` // 漂分 输赢分
	MaScore       string                          `json:"maScore"`   // 码分 输赢分
	LiangDaoScore string                          `json:"LDScore"`   // 亮倒 输赢分
	GangScore     string                          `json:"gangScore"` // 杠
	IsLiangDao    bool                            `json:"liangDao"`  // 是否亮倒
	Piao          int64                           `json:"piao"`      // 座位上的漂
	KanScore      string                          `json:"kanScore"`  // 坎 输赢分
	//PaoScore      string                          `json:"paoS"`      // 跑 输赢分
	//QiaScore      string                          `json:"qiaS"`      // 恰 输赢分
	//MoScore       string                          `json:"moS"`       // 摸 输赢分
	//BaScore       string                          `json:"baS"`       // 八 输赢分
...
type BroadRoundGameOver struct {
	...
	MaPaiArr           []int8       `json:"maPai"`           // 码牌
	IsShangLou         bool         `json:"isShangLou"`      // 是否上楼
}
...
// 摇筛子
const ID_YaoShaiZi = 20110

type CS_YaoShaiZi struct {
	SZ [2]int `json:"sz"`
}
