...
type KaWuXingPlayLogic struct {
	...
	//LastRoundIsHuangZhuang bool               // 上局 是否 黄庄
	//LastRoundIsYiPaoDuoX   bool               // 上局 是否 一炮多响

	// 小局 待清理 成员
	shangLouDianShu      [2]int
...
func (this *KaWuXingPlayLogic) CleanRoundData() {
	this.shangLouDianShu[0] = 0
	this.shangLouDianShu[1] = 0
...
func (this *KaWuXingPlayLogic) OnGameStart(pro *mateProto.MessageMaTe) int32 {
	...
	if this.PlayRule.IsShangLou {
		this.shangLouDianShu[0] = rand.Intn(6) + 1
		this.shangLouDianShu[1] = rand.Intn(6) + 1
		this.Table.BroadCastGameEvent(ID_YaoShaiZi, &CS_YaoShaiZi{this.shangLouDianShu})
	}
...
func (this *KaWuXingPlayLogic) DoGang(seatData *qpTable.SeatData, mjSeatData *gameMaJiang.MJSeat, gangPai *gameMaJiang.CS_Gang, gangType int32) {
	...
	if this.PlayRule.IsShangLou == true && this.shangLouDianShu[0] == this.shangLouDianShu[1] {
		gangScore *= 2
	}
...
func (this *KaWuXingPlayLogic) getPaiXing(kwxSeat *KWXSeat) []*gameMaJiang.HuPaiXing {
	...
	if this.PlayRule.IsShangLou == true && this.shangLouDianShu[0] == this.shangLouDianShu[1] {
		tempPXFS := &gameMaJiang.HuPaiXing{PaiXing: HuPX_ShangLou, FanShu: 2}
		paiXingArr = append(paiXingArr, tempPXFS)
		mjSeat.HuScore *= tempPXFS.FanShu
	}
