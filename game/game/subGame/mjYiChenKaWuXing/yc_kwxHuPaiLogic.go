...
func (this *kwxHuLogic) getPaoQiaMoBa_Qia_Ba(info *[]*gameMaJiang.OperationPaiInfo, isHaoHua7Dui bool) (int64, int64) {
	this.paiArr = this.paiArrBak

	gangHuaSe := []int8{0, 0, 0, 0}
	qia, ba := int64(0), int64(0)
	for _, v := range *info {
		//qia += 1
		if v.OperationPXItem == gameMaJiang.OPX_MING_GANG ||
			v.OperationPXItem == gameMaJiang.OPX_AN_GANG ||
			v.OperationPXItem == gameMaJiang.OPX_BU_GANG {
			gangHuaSe[v.PaiArr[0]>>4] += 4
			qia += 1
		} else {
			gangHuaSe[v.PaiArr[0]>>4] += 3
		}
	}

	maxHuaSePaiCount := int8(0)
	for huaSe := gameMaJiang.MinHuaSe; huaSe <= gameMaJiang.MaxHuaSe; huaSe++ {
		if this.paiArr[huaSe][0] < 1 {
			continue
		}
		if this.paiArr[huaSe][0]+gangHuaSe[huaSe] > maxHuaSePaiCount {
			maxHuaSePaiCount = this.paiArr[huaSe][0] + gangHuaSe[huaSe]
		}

		for dianShu := gameMaJiang.MinDianShu_1; dianShu <= gameMaJiang.MaxDianShu_9; dianShu++ {
			if this.paiArr[huaSe][dianShu] >= 3 && isHaoHua7Dui == false {
				qia += 1
			}
		}
	}

	if maxHuaSePaiCount >= 8 {
		ba = int64(maxHuaSePaiCount) - 8 + 1
	}
