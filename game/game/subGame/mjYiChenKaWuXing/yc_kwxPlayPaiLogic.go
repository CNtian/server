...
func (this *KaWuXingPlayLogic) OnPeng(pro *mateProto.MessageMaTe) int32 {
	...
	if len(this.huSeatMap) > 0 && (pengPaiSeat.(*KWXSeat).MJSeat.OperationItem&gameMaJiang.OPI_HU == gameMaJiang.OPI_HU) {
...
func (this *KaWuXingPlayLogic) OnGang(pro *mateProto.MessageMaTe) int32 {
	...
	if len(this.huSeatMap) > 0 && (gangPaiSeat.(*KWXSeat).MJSeat.OperationItem&gameMaJiang.OPI_HU == gameMaJiang.OPI_HU) {
		return mateProto.Err_FindHuPlayer
	}
...
func (this *KaWuXingPlayLogic) DoGang(seatData *qpTable.SeatData, mjSeatData *gameMaJiang.MJSeat, gangPai *gameMaJiang.CS_Gang, gangType int32) {
	...
	getScoreCategory := gameMaJiang.Nil
	if gangType == 1 {
		getScoreCategory = gameMaJiang.MingGang
		playPaiMJSeat := this.Table.SeatArr[this.CurPlaySeatNum].GetXSeatData(0).(*gameMaJiang.MJSeat)
		playPaiMJSeat.PlayPai = playPaiMJSeat.PlayPai[:len(playPaiMJSeat.PlayPai)-1]
		...
		getScoreCategory = gameMaJiang.AnGang
		// 删除手牌
		mjSeatData.DeleteShouPai(gangPai.Pai)
		mjSeatData.DeleteShouPai(gangPai.Pai)
		mjSeatData.DeleteShouPai(gangPai.Pai)
		...
		getScoreCategory = gameMaJiang.BuGang
	...
	// 算 杠分
	if gangType == 1 {
		this.changeGangScore2(seatData.Number, this.CurPlaySeatNum, gangScore, getScoreCategory)
	} else {
		this.changeGangScore(seatData.Number, gangScore, getScoreCategory)
	}
...
func (this *KaWuXingPlayLogic) getPaiXing(kwxSeat *KWXSeat) []*gameMaJiang.HuPaiXing {
	...
	if this.PlayRule.IsPaoQiaMoBa == true {
		kwxSeat.qiaCount, kwxSeat.baCount = this.HuLogic.getPaoQiaMoBa_Qia_Ba(&mjSeat.OperationPai, isHaoHua7Dui)
	}
...
func (this *KaWuXingPlayLogic) changeGangScore(win qpTable.SeatNumber, score float64, category int) {
	...
	rec := qpTable.GameScoreRec{Category: category}
	...
	for i, v := range this.Table.SeatArr {
		...
		rec.Score, rec.TargetUID = -score, []qpTable.SeatNumber{win}
	...
	rec.Score, rec.TargetUID = allScore, giveSeatArr
...
func (this *KaWuXingPlayLogic) changeGangScore2(win, lose qpTable.SeatNumber, score float64, category int) {
	rec := qpTable.GameScoreRec{Category: category, Score: score}
