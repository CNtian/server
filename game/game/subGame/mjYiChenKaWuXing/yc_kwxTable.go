...
func (this *GameKWXTable) RoundOver() {
	...
	// 游戏分
	{
		...
		for _, v := range this.table.SeatArr {
			...
			v.(*KWXSeat).totalGangScore += v.(*KWXSeat).GangScore
		}
	}
	...
	for _, v := range this.table.SeatArr {
		...
		for _, f := range v.(*KWXSeat).delayRec {
			f()
		}
		tempHuSeat := HuPaiSeat{
			...
			PaoScore:      commonDef.Float64ToString(v.(*KWXSeat).PaoScore),
			QiaScore:      commonDef.Float64ToString(v.(*KWXSeat).QiaScore),
			MoScore:       commonDef.Float64ToString(v.(*KWXSeat).MoScore),
			BaScore:       commonDef.Float64ToString(v.(*KWXSeat).BaScore),
			KouPai:        v.(*KWXSeat).GetKouPai(),
			IsLiangDao:    isLiangDao,
			Piao:          v.(*KWXSeat).PiaoScore,
			GameScoreStep: tempSeat.GameScoreRecStep,
			PaiXingScore:  commonDef.Float64ToString(v.(*KWXSeat).PaiXingScore),
		}
...
func (this *GameKWXTable) handleDaJieSuan() {
	...
	for _, v := range this.table.SeatArr {
		...
		tempSeat := &GameOverSeatData{
			...
			IsMaxWin:     false,
			GangScore:    commonDef.Float64ToString(v.(*KWXSeat).totalGangScore),
			PiaoScore:    commonDef.Float64ToString(v.(*KWXSeat).totalPiaoScore),
			MaScore:      commonDef.Float64ToString(v.(*KWXSeat).totalMaScore),
			PaiXinScore:  commonDef.Float64ToString(v.(*KWXSeat).totalPaiXinScore),
		}
...
func (this *GameKWXTable) OnHu(pro *mateProto.MessageMaTe) int32 {
	...
	for _, v := range this.table.SeatArr {
		...
		if (tempMJSeat.OperationItem & gameMaJiang.OPI_HU) == gameMaJiang.OPI_HU {
			...
			this.table.SendMsgToSeat(mjSeatData.SeatData.Number, pro)
			return mateProto.Err_Success
		}
	}
...
func (this *GameKWXTable) calculateScore(huMode int32, huSeatNum qpTable.SeatNumber) []int8 {
	...
	if huMode == 1 {
		...
		//huScoreInt64 := huMJSeat.HuScore
		if this.gameRule.IsPaoQiaMoBa == true {
			// 跑
			//if huScoreInt64 < this.gameRule.FengDingFanShu {
			//	if huScoreInt64+1 >= this.gameRule.FengDingFanShu {
			//		huScoreInt64 = this.gameRule.FengDingFanShu
			//	} else {
			//		huScoreInt64 += 1
			//	}
			//	huSeat.PaoScore = float64(1) * this.gameRule.MultipleFloat64
			//}
			huSeat.PaoScore = float64(1) * this.gameRule.MultipleFloat64
			// 恰
			//if huScoreInt64 < this.gameRule.FengDingFanShu {
			//	if huScoreInt64+huSeat.qiaCount >= this.gameRule.FengDingFanShu {
			//		tempQia := this.gameRule.FengDingFanShu - huScoreInt64
			//		huScoreInt64 = this.gameRule.FengDingFanShu
			//		huSeat.QiaScore = float64(tempQia) * this.gameRule.MultipleFloat64
			//	} else {
			//		huScoreInt64 += huSeat.qiaCount
			//		huSeat.QiaScore = float64(huSeat.qiaCount) * this.gameRule.MultipleFloat64
			//	}
			//}
			huSeat.QiaScore = float64(huSeat.qiaCount) * this.gameRule.MultipleFloat64
			// 摸
			...
			//		huScoreInt64 += 1
			//	}
			//	huSeat.MoScore = float64(1) * this.gameRule.MultipleFloat64
			//}
			huSeat.MoScore = float64(1) * this.gameRule.MultipleFloat64
			// 八
			//if huScoreInt64 < this.gameRule.FengDingFanShu {
			//	if huScoreInt64+huSeat.baCount >= this.gameRule.FengDingFanShu {
			//		tempQia := this.gameRule.FengDingFanShu - huScoreInt64
			//		huScoreInt64 = this.gameRule.FengDingFanShu
			//		huSeat.BaScore = float64(tempQia) * this.gameRule.MultipleFloat64
			//	} else {
			//		huScoreInt64 += huSeat.baCount
			//		huSeat.BaScore = float64(huSeat.baCount) * this.gameRule.MultipleFloat64
			//	}
			//}
			huSeat.BaScore = float64(huSeat.baCount) * this.gameRule.MultipleFloat64
		}
		huScore := float64(huMJSeat.HuScore) * this.gameRule.MultipleFloat64
		pqmbScore := huSeat.PaoScore + huSeat.QiaScore + huSeat.MoScore + huSeat.BaScore

		losePlayerArr, _ := make([]qpTable.SeatNumber, 0, 2), make([]qpTable.SeatNumber, 0, 2)
		totalMaScore, _ := float64(0), float64(0)
		piaoFuncArr := make([]func(), 0, 2)
		for _, v := range this.table.SeatArr {
			if v == nil {
				continue
			}
			...
			losePlayer := v.(*KWXSeat)
			if this.gameRule.IsPaoQiaMoBa == true {
				losePlayer.PaoScore = 0 - huSeat.PaoScore
				losePlayer.QiaScore = 0 - huSeat.QiaScore
				losePlayer.MoScore = 0 - huSeat.MoScore
				losePlayer.BaScore = 0 - huSeat.BaScore
			}
			...
			tempPaiXing := huSeat.MJSeat.HuPaiXing
			...
			if this.playPaiLogic.HuLogic.isLiangDao(huSeat) == false &&
				...
				tempPaiXing = append(tempPaiXing, &gameMaJiang.HuPaiXing{PaiXing: HuPX_LiangDao, FanShu: 2})
			}

			if pqmbScore > 0 {
				tempPaiXing = append(tempPaiXing, &gameMaJiang.HuPaiXing{PaiXing: HuPX_PaoQiaMoBa, FanShu: 1})
			}

			tempScore := huScore + tempPiaoScore + tempMaScore + liangDaoScore + pqmbScore

			v.(*KWXSeat).totalPaiXinScore -= huScore + liangDaoScore
			v.(*KWXSeat).totalPiaoScore -= tempPiaoScore
			v.(*KWXSeat).totalMaScore -= tempMaScore

			v.(*KWXSeat).PaiXingScore -= huScore
			huSeat.PaiXingScore += huScore

			v.GetSeatData().RoundScore -= tempScore
			huSeat.GetSeatData().RoundScore += tempScore

			huSeat.totalPaiXinScore += huScore + liangDaoScore
			huSeat.totalPiaoScore += tempPiaoScore
			huSeat.totalMaScore += tempMaScore

			totalMaScore += tempMaScore
			losePlayerArr = append(losePlayerArr, losePlayer.MJSeat.SeatData.Number)

			loseRec := qpTable.GameScoreRec{Category: gameMaJiang.ZiMo,
				Score:     huScore + liangDaoScore + pqmbScore,
				PaiXing:   tempPaiXing,
				TargetUID: []qpTable.SeatNumber{huSeat.MJSeat.SeatData.Number}}
			losePlayer.MJSeat.SeatData.PutGameScoreItem(&loseRec, -1)

			piaoFenRec := qpTable.GameScoreRec{Category: gameMaJiang.PiaoFen,
				Score:     tempPiaoScore,
				TargetUID: []qpTable.SeatNumber{huSeat.MJSeat.SeatData.Number}}
			losePlayer.MJSeat.SeatData.PutGameScoreItem(&piaoFenRec, -1)

			maFenRec := qpTable.GameScoreRec{Category: gameMaJiang.MaFen,
				Score:     tempMaScore,
				TargetUID: []qpTable.SeatNumber{huSeat.MJSeat.SeatData.Number}}
			losePlayer.MJSeat.SeatData.PutGameScoreItem(&maFenRec, -1)

			win_Rec := qpTable.GameScoreRec{Category: gameMaJiang.ZiMo,
				Score:     huScore + liangDaoScore + pqmbScore,
				PaiXing:   tempPaiXing,
				TargetUID: []qpTable.SeatNumber{losePlayer.MJSeat.SeatData.Number}}
			huSeat.MJSeat.SeatData.PutGameScoreItem(&win_Rec, 1)

			win_piaoFenRec := qpTable.GameScoreRec{Category: gameMaJiang.PiaoFen,
				Score:     tempPiaoScore,
				TargetUID: []qpTable.SeatNumber{losePlayer.MJSeat.SeatData.Number}}
			piaoFuncArr = append(piaoFuncArr, func() {
				huSeat.MJSeat.SeatData.PutGameScoreItem(&win_piaoFenRec, 1)
			})
		}

		for _, f := range piaoFuncArr {
			f()
		}

		win_maFenRec := qpTable.GameScoreRec{Category: gameMaJiang.MaFen,
			Score:     totalMaScore,
			TargetUID: losePlayerArr}
		huSeat.MJSeat.SeatData.PutGameScoreItem(&win_maFenRec, 1)
	...
	if this.gameRule.IsPaoQiaMoBa == true {
		...
		dianPaoSeat.PaoScore += 0 - huSeat.PaoScore
		dianPaoSeat.QiaScore += 0 - huSeat.QiaScore
		dianPaoSeat.MoScore += 0 - huSeat.MoScore
		dianPaoSeat.BaScore += 0 - huSeat.BaScore
	...
	winScore := piaoScore + huScore + liangDaoScore + pqmbScore

	huSeat.totalPaiXinScore += huScore + liangDaoScore
	huSeat.totalPiaoScore += piaoScore
	huSeat.PaiXingScore += huScore + liangDaoScore

	dianPaoSeat.totalPaiXinScore -= huScore + liangDaoScore
	dianPaoSeat.totalPiaoScore -= piaoScore
	dianPaoSeat.PaiXingScore -= huScore + liangDaoScore
	...
	{
		loseRec := qpTable.GameScoreRec{Category: gameMaJiang.DianPao,
			Score:     huScore + liangDaoScore + pqmbScore,
			PaiXing:   tempPaiXing,
			TargetUID: []qpTable.SeatNumber{huSeat.MJSeat.SeatData.Number}}
		dianPaoSeat.MJSeat.SeatData.PutGameScoreItem(&loseRec, -1)

		piaoFenRec := qpTable.GameScoreRec{Category: gameMaJiang.PiaoFen,
			Score:     piaoScore,
			TargetUID: []qpTable.SeatNumber{huSeat.MJSeat.SeatData.Number}}
		dianPaoSeat.delayRec = append(dianPaoSeat.delayRec, func() {
			dianPaoSeat.MJSeat.SeatData.PutGameScoreItem(&piaoFenRec, -1)
		})

		winRec := qpTable.GameScoreRec{Category: gameMaJiang.DianPao,
			Score:     huScore + liangDaoScore + pqmbScore,
			PaiXing:   tempPaiXing,
			TargetUID: []qpTable.SeatNumber{dianPaoSeat.MJSeat.SeatData.Number}}
		huSeat.MJSeat.SeatData.PutGameScoreItem(&winRec, 1)

		winPiaoFenRec := qpTable.GameScoreRec{Category: gameMaJiang.PiaoFen,
			Score:     piaoScore,
			TargetUID: []qpTable.SeatNumber{dianPaoSeat.MJSeat.SeatData.Number}}
		huSeat.MJSeat.SeatData.PutGameScoreItem(&winPiaoFenRec, 1)
	}
