...
type NiuNiuRule struct {
	...
	XiaZhuOpt int32 `json:"xiaZhu"` // 下注 底分 // 0:1/3/5 1:2/5/10 2:5/10/15 3:10/20/40
	XiaZhuArr [4][3]int32

	//IsNormal bool `json:"isNormal"`
	IsSuper  bool `json:"isSuper"`
	IsTuiZhu bool `json:"isTuiZhu"`

	FirstRoundReady int32 `json:"firstRoundReady"` // 0(所有人),2 ,3 ,4
	//ZhuangMinScoreText    string  `json:"zhuangMinScore"`
	//ZhuangMinScoreFloat64 float64 `json:"-"`

	IsDoublePai bool `json:"isDoublePai"`
	IsLaiZi     int  `json:"lz"` // 0：不用赖子 1:随机赖子  2:双王赖子  3：随机 + 双王 赖子
...
func (this *NiuNiuRule) CheckField() error {
	...
	this.XiaZhuArr[0][1] = 3
	this.XiaZhuArr[0][2] = 5

	this.XiaZhuArr[1][0] = 2
	this.XiaZhuArr[1][1] = 5
	this.XiaZhuArr[1][2] = 10

	this.XiaZhuArr[2][0] = 5
	this.XiaZhuArr[2][1] = 10
	this.XiaZhuArr[2][2] = 15

	this.XiaZhuArr[3][0] = 10
	this.XiaZhuArr[3][1] = 20
	this.XiaZhuArr[3][2] = 40
...
func newNiuNiuSeat(playerID qpTable.PlayerID, seatNumber qpTable.SeatNumber) (qpTable.QPSeat, error) {
	...
	pdkSeat := &NiuNiuMPQZSeat{seatData: baseSeat}
	pdkSeat.CleanRoundData()

	return pdkSeat, nil
}

func NewNiuNiuMingPaiQiangZhuangTable(tableNum int32, gameRuleCfg, tableCfg, clubCfg string) (*NiuNiuMPQZTable, int32, string) {
	...
	// 俱乐部规则
	//if len(clubCfg) > 0 {
	//	zhuangMinScore := gjson.Get(clubCfg, "zhuangMinScore")
	//	if zhuangMinScore.Exists() == false {
	//		return nil, mateProto.Err_CreateTableParam, "zhuangMinScore"
	//	}
	//	t.gameRule.ZhuangMinScoreFloat64, err = strconv.ParseFloat(zhuangMinScore.String(), 64)
	//	if err != nil {
	//		return nil, mateProto.Err_CreateTableParam, err.Error()
	//	}
	//	if math.IsNaN(t.gameRule.ZhuangMinScoreFloat64) == true {
	//		return nil, mateProto.Err_CreateTableParam, "zhuangMinScore"
	//	}
	//}
