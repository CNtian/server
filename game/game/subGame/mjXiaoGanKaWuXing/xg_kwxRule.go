...
type KWXPlayRule struct {
	...
	IsShuKang        bool `json:"shuKang"`        // 数坎
	IsLess12NotLiang bool `json:"less12NotLiang"` // 低于12张不能亮
...
func NewGameKWXTable(tableNum int32, gameRuleCfg, tableCfg string) (*GameKWXTable, int32, string) {
	...
	t.playPaiLogic.liangDaoSeatNumMap = make(map[qpTable.SeatNumber]*KWXSeat)
	t.playPaiLogic.noLiangDaoSeatNumMap = make(map[qpTable.SeatNumber]*KWXSeat)
