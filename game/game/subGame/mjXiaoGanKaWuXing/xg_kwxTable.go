...
func (this *GameKWXTable) RoundOver() {
	...
	for _, v := range this.table.SeatArr {
		...
		tempHuSeat := HuPaiSeat{
			...
			KanScore:      commonDef.Float64ToString(v.(*KWXSeat).KanScore),
...
func (this *GameKWXTable) calculateScore(huMode int32, huSeatNum qpTable.SeatNumber) []int8 {
	...
	if huMode == 1 {
		...
		for _, v := range this.table.SeatArr {
			...
			kanScore := float64(huSeat.KanShu) * this.gameRule.MultipleFloat64
			huSeat.KanScore += kanScore
			v.(*KWXSeat).KanScore -= kanScore

			tempScore := huScore + tempPiaoScore + tempMaScore + liangDaoScore + kanScore
	...
	kanScore := float64(huSeat.KanShu) * this.gameRule.MultipleFloat64
	huSeat.KanScore += kanScore
	dianPaoSeat.KanScore -= kanScore

	winScore := piaoScore + huScore + liangDaoScore + kanScore
