...
func (this *KaWuXingPlayLogic) OnMessage(pro *mateProto.MessageMaTe) int32 {
	...
	case ID_CustomNextPai:
		rspCode = this.onCustomNextPai(pro)
	case ID_GetRemainingPai:
		rspCode = this.onGetRemainingPai(pro)
...
func (this *KaWuXingPlayLogic) faShouPai() {
	...
	this.Table.GameTimer.PutTableTimer(TIMER_FaPai, 2*1000, func() {
		// 庄家摸牌
...
func (this *KaWuXingPlayLogic) OnPeng(pro *mateProto.MessageMaTe) int32 {
	...
	if this.FindPriorityOperation(seatData.Number, heightPriority) == true {
		...
		this.Table.SendGameEventToSeat(
			...
			&gameMaJiang.SC_NoticeOperation{SeatNumber: int32(seatData.Number),
				OperationID: "",
				Operation:   0,
				Pai:         0,
				GangPai:     []int8{},
...
func (this *KaWuXingPlayLogic) DoPeng(seatData *qpTable.SeatData, mjSeatData *gameMaJiang.MJSeat, pengPai *gameMaJiang.CS_Peng) {
	...
	operInfo := &gameMaJiang.OperationPaiInfo{OperationPXItem: gameMaJiang.OPX_PENG,
		...
		Name:              playPaiMJSeat.SeatData.Player.Nick}
...
func (this *KaWuXingPlayLogic) DoGang(seatData *qpTable.SeatData, mjSeatData *gameMaJiang.MJSeat, gangPai *gameMaJiang.CS_Gang, gangType int32) {
	...
	if gangType == 1 {
		...
		// 更新 操作区域
		for _, v := range mjSeatData.OperationPai {
			if v.OperationPXItem == gameMaJiang.OPX_PENG && v.PaiArr[0] == gangPai.Pai {
	...
	// 算 杠分
	if gangType == 1 {
		this.changeGangScore2(seatData.Number, this.CurPlaySeatNum, &rec)
	} else {
		this.changeGangScore(seatData.Number, &rec)
	}
...
func (this *KaWuXingPlayLogic) OnGuo(pro *mateProto.MessageMaTe) int32 {
	...
	if this.FindPriorityOperation(seatData.Number, operationCode) == true {
		...
		this.Table.SendGameEventToSeat(
			qpTable.SeatNumber(proMsg.SeatNumber),
			gameMaJiang.ID_NoticeOperation,
			&proMsg)
		return mateProto.Err_Success
	}
	...
	// 点炮胡时，有胡的人取消
	if this.isHued == true && this.dianPaoSeatNum != qpTable.INVALID_SEAT_NUMBER {
		this.MoPaiOperation(this.dianPaoSeatNum)
		return mateProto.Err_Success
	}
...
func (this *KaWuXingPlayLogic) GetPlayPaiOperation(playSeatNum qpTable.SeatNumber, playPai int8, gangCount int32) []*KWXSeat {
	...
	for i, v := range this.Table.SeatArr {
		...
		var oper gameMaJiang.PlayerMJOperation
		if mjSeat.IsGuoHu == false && this.IsDianPaoHu(v.(*KWXSeat), playPai, false, playPaiSeatIsGang) == true {
			oper |= gameMaJiang.OPI_HU
		}
		if v.(*KWXSeat).LiangDaoMap == nil && this.IsPeng(mjSeat.ShouPai, playPai) == true {
...
func (this *KaWuXingPlayLogic) getPaiXing(kwxSeat *KWXSeat) []*gameMaJiang.HuPaiXing {
	...
	if mjSeat.HuScore > this.PlayRule.FengDingFanShu {
		mjSeat.HuScore = this.PlayRule.FengDingFanShu
	}
...
func (this *KaWuXingPlayLogic) changeGangScore(win qpTable.SeatNumber, rec *qpTable.GameScoreRec) {
	...
	for i, v := range this.Table.SeatArr {
		...
		if v.GetSeatData().IsAssignSeatState(SS_Suspend) == true {
			continue
		}

		v.(*KWXSeat).GangScore -= rec.Score
		...
		allScore += rec.Score
		this.Table.SeatArr[win].(*KWXSeat).GangScore += rec.Score
...
func (this *KaWuXingPlayLogic) changeGangScore2(win, lose qpTable.SeatNumber, v *qpTable.GameScoreRec) {

	this.Table.SeatArr[win].(*KWXSeat).GangScore += v.Score
	this.Table.SeatArr[win].GetSeatData().RoundScore += v.Score
	this.Table.SeatArr[win].GetSeatData().PutGameScoreItem(v, 1)

	this.Table.SeatArr[lose].(*KWXSeat).GangScore -= v.Score
	this.Table.SeatArr[lose].GetSeatData().RoundScore -= v.Score
	this.Table.SeatArr[lose].GetSeatData().PutGameScoreItem(v, -1)

	msgBody := protoGameBasic.BroadcastPlayerScoreChanged{
		WinnerSeatNum: int32(win),
		LoserSeatNum:  make([]int32, 0, 1),
		Score:         commonDef.Float64ToString(v.Score)}
...
// 激活托管
func (this *KaWuXingPlayLogic) OnActiveTrusteeship(msg *mateProto.MessageMaTe) int32 {
...
// 获取正在玩的下一位玩家
func (this *KaWuXingPlayLogic) getNextPlayingSeat(cur qpTable.SeatNumber) qpTable.QPSeat {

	for i := cur + 1; int(i) < len(this.Table.SeatArr); i++ {
		if this.Table.SeatArr[i] == nil {
			continue
		}
		t := this.Table.SeatArr[i].(*KWXSeat)
		if t.MJSeat.SeatData.IsAssignSeatState(qpTable.SS_Playing) == true &&
			t.MJSeat.SeatData.IsAssignSeatState(SS_Suspend) == false {
...
func (this *KaWuXingPlayLogic) isGameOver() bool {
	if this.PaiMgr.GetTheRestOfPaiCount() < 1 ||
		this.plays < 2 {
		return true
	}
