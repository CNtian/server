...
type PokerDouDiZhuTable struct {
	PaiMgr   *pokerTable.PokerPaiBaseMgr // 牌的管理器
	logic    PokerDouDizhuLogic
	table    *qpTable.QPTable
	gameRule PokerDouDiZhuRule
...
// 清空每一小局数据
func (this *PokerDouDiZhuTable) CleanRoundData() {
...
func (this *PokerDouDiZhuTable) SetTableNumber(tabNumber int32) {
	this.table.TableNum = tabNumber
}

func (this *PokerDouDiZhuTable) GetStatus() int32 {
	return int32(this.table.Status)
}

func (this *PokerDouDiZhuTable) ParseTableOptConfig(gameRuleCfg string) (rspCode int32, err error) {
...
func (this *PokerDouDiZhuTable) GetMaxRound() int32 {
	return this.gameRule.MaxRoundCount
}

func (this *PokerDouDiZhuTable) OnMessage(msg *mateProto.MessageMaTe) int32 {
...
func (this *PokerDouDiZhuTable) GetBaseQPTable() *qpTable.QPTable {
	return this.table
}

func (this *PokerDouDiZhuTable) onPrivateJoinTable(msg *mateProto.MessageMaTe) int32 {
	...
	if rspCode >= 0 {
		this.table.SeatArr[rspCode].(*DouDiZhuSeat).paiLogic.rule = &this.gameRule
...
func (this *PokerDouDiZhuTable) onClubJoinTable(msg *mateProto.MessageMaTe) int32 {
...
func (this *PokerDouDiZhuTable) onPlayerLeave(pro *mateProto.MessageMaTe) int32 {
...
func (this *PokerDouDiZhuTable) onReady(pro *mateProto.MessageMaTe) int32 {
...
func (this *PokerDouDiZhuTable) OnPlayerPlayPai(pro *mateProto.MessageMaTe) int32 {
	...
	if len(operPlayPai.ChuPai) == 1 {
		...
		if nextSeat.(*DouDiZhuSeat).shouPaiCount == 1 {
	...
	if len(operPlayPai.ChuPai) > 0 {
		...
		if this.lastPlaySeatNumber != pdkSeat.seatData.Number {
			lastPlaySeat := this.table.SeatArr[this.lastPlaySeatNumber].(*DouDiZhuSeat)
	...
	for i := int32(0); i < this.gameRule.MaxPlayer; i++ {
		...
		if this.findGreaterPai(tempSeat.(*DouDiZhuSeat)) == true {
			//tempTestArr := make([]int8, len(tempSeat.(*DouDiZhuSeat).autoPlayPaiArr))
			//for i, v := range tempSeat.(*DouDiZhuSeat).autoPlayPaiArr {
			//	tempTestArr[i] = v & 0x0F
			//}
			//commonDef.LOG_Info("预计出牌：=", tempTestArr)

			nextSeatNumber = tempSeat.(*DouDiZhuSeat).seatData.Number
			break
		}
	...
	// 检查是否 能 一次性 出完
	{
		tempLogic := PokerDouDizhuLogic{rule: &this.gameRule}
		nextSeat := this.table.SeatArr[nextSeatNumber].(*DouDiZhuSeat)
...
func (this *PokerDouDiZhuTable) onGameStart(pro *mateProto.MessageMaTe) int32 {
	if this.table.IsAssignTableState(qpTable.TS_WaitingPlayerEnter) == false &&
	...
	this.PaiMgr.XiPai(this.table.GetCurSeatCount()-this.table.LookerCount, 17)

	switch this.gameRule.LaiZiMode {
	case 2:
	...
	for i, v := range this.table.SeatArr {
		...
		paiArr := this.PaiMgr.GetGroupPai(int32(seat.Number), 17,
			func(pai int8) {
	...
	//	}
	//	for k, _ := range this.table.SeatArr[seatNumber].(*DouDiZhuSeat).shouPai {
	//		luckPai = k
	//		break
	//	}
...
func (this *PokerDouDiZhuTable) RoundOverFun() {
	...
	winSeatData := this.table.SeatArr[this.lastPlaySeatNumber].(*DouDiZhuSeat)
	...
	for _, v := range this.GetBaseQPTable().SeatArr {
		...
		if this.lastPlaySeatNumber == curSeatNumber {
			v.(*DouDiZhuSeat).winCount += 1
			continue
		}
		v.(*DouDiZhuSeat).loseCount += 1

		var tempScore float64
		// 手牌数 > 1
		if v.(*DouDiZhuSeat).shouPaiCount > 1 {
			tempScore = float64(v.(*DouDiZhuSeat).shouPaiCount)
			// 赢家中鸟
			...
			// 春天
			if this.bankerSeatNum == this.lastPlaySeatNumber && v.(*DouDiZhuSeat).playPaiCount < 1 {
				v.(*DouDiZhuSeat).chunTianCount += 1
				v.(*DouDiZhuSeat).isChunTian = true
				tempScore *= 2
			}
			// 反春
			if curSeatNumber == this.bankerSeatNum && v.(*DouDiZhuSeat).playPaiCount < 2 {
				v.(*DouDiZhuSeat).isFanChun = true
				tempScore *= 2
			}
		...
		v.(*DouDiZhuSeat).TotalBombScore += v.(*DouDiZhuSeat).bombScore
		v.(*DouDiZhuSeat).TotalPaiScore -= tempScore
		v.(*DouDiZhuSeat).PaiScore = -tempScore
		// 输家 -分
	...
	for _, v := range this.GetBaseQPTable().SeatArr {
		...
		roundSeatScoreArr = append(roundSeatScoreArr, &RoundSeatScore{
			...
			Pai:           v.(*DouDiZhuSeat).GetAllPai(),
			BombScore:     commonDef.Float64ToString(v.(*DouDiZhuSeat).bombScore),
			GameScore:     commonDef.Float64ToString(seat.RoundScore),
			SeatScore:     commonDef.Float64ToString(seat.SeatScore),
			IsNiaoPai:     isNiaoPai,
			IsChunTian:    v.(*DouDiZhuSeat).isChunTian,
			IsFanChun:     v.(*DouDiZhuSeat).isFanChun,
			GameScoreStep: seat.GameScoreRecStep,
			RecChuPai:     v.(*DouDiZhuSeat).recChuPai})
...
func (this *PokerDouDiZhuTable) handleXiaoJieSuan() {
	if this.table.CurXRound < 1 {
		return
	}
	...
	for _, v := range this.GetBaseQPTable().SeatArr {
		...
		roundSeatScoreArr = append(roundSeatScoreArr, &RoundSeatScore{
			...
			GameScoreStep: seat.GameScoreRecStep,
			RecChuPai:     v.(*DouDiZhuSeat).recChuPai,
			PaiXinScore:   commonDef.Float64ToString(v.(*DouDiZhuSeat).PaiScore)})
	}
...
// 大结算
func (this *PokerDouDiZhuTable) handleDaJieSuan() {
	if this.table.CurXRound < 1 {
		return
	}
...
func (this *PokerDouDiZhuTable) onTableExpire(pro *mateProto.MessageMaTe) int32 {
...
func (this *PokerDouDiZhuTable) onTableData(pro *mateProto.MessageMaTe) int32 {
	...
	for _, v := range this.table.SeatArr {
		...
		pkPDKSeat := v.GetXSeatData(0).(*DouDiZhuSeat)
	...
	pdkSeat := seat.GetXSeatData(0).(*DouDiZhuSeat)
...
func (this *PokerDouDiZhuTable) NoticeOperation(seatNumber qpTable.SeatNumber, item OperationItem) {
	pdkSeat := this.table.SeatArr[seatNumber].(*DouDiZhuSeat)
...
// 座位游戏分变化
func (this *PokerDouDiZhuTable) ChangeBombScore(win qpTable.SeatNumber, score float64) {
	...
	for i, v := range this.table.SeatArr {
		...
		if qpTable.SeatNumber(i) == win {
			this.table.SeatArr[win].(*DouDiZhuSeat).bombCount += 1
			continue
		}
		...
		v.(*DouDiZhuSeat).bombScore -= score

		tempScore += score
		this.table.SeatArr[win].GetSeatData().RoundScore += score
		this.table.SeatArr[win].(*DouDiZhuSeat).bombScore += score
...
// 检查操作
func (this *PokerDouDiZhuTable) checkOperation(playerID int64, operationID string) (qpTable.QPSeat, int32) {
...
func (this *PokerDouDiZhuTable) timerAutoTrusteeship() {
...
func (this *PokerDouDiZhuTable) findGreaterPai(curPDKPlayPaiSeat *DouDiZhuSeat) bool {
	lastPDKPlayPaiSeat := this.table.SeatArr[this.lastPlaySeatNumber].(*DouDiZhuSeat)
...
func (this *PokerDouDiZhuTable) autoTrusteeshipPlayPai() {
	...
	curPDKPlayPaiSeat := curPlayPaiSeat.(*DouDiZhuSeat)
	if this.curPlaySeatNumber == this.lastPlaySeatNumber {
		...
		if nextPlaySeat.(*DouDiZhuSeat).shouPaiCount == 1 {
		...
		tempPlayPaiArr = curPlayPaiSeat.(*DouDiZhuSeat).autoPlayPaiArr
	}
...
func (this *PokerDouDiZhuTable) onCancelTrusteeship(msg *mateProto.MessageMaTe) int32 {
...
func (this *PokerDouDiZhuTable) onCustomShouPai(pro *mateProto.MessageMaTe) int32 {
	...
	if len(msgCustomShouPai.ShouPai) > 17 {
...
func (this *PokerDouDiZhuTable) onDissolveTableVote(pro *mateProto.MessageMaTe) int32 {
...
func (this *PokerDouDiZhuTable) onForceDissolveTable(pro *mateProto.MessageMaTe) int32 {
	rspCode := this.table.OnMessage(pro)
...
// 激活托管
func (this *PokerDouDiZhuTable) OnActiveTrusteeship(msg *mateProto.MessageMaTe) int32 {
